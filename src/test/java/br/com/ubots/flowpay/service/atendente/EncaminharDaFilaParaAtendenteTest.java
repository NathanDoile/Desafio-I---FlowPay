...
class EncaminharDaFilaParaAtendenteTest {
    ...
    void deveEncaminharSolicitacaoParaAtendenteELotarEle(){
        ...
        Atendente atendente = atendente();
        atendente.getSolicitacoes().add(Solicitacao.builder().statusSolicitacao(EM_ATENDIMENTO).build());
        atendente.getSolicitacoes().add(Solicitacao.builder().statusSolicitacao(EM_ATENDIMENTO).build());
    ...
    @Test
    @DisplayName("Não deve encaminhar uma solicitação para um atendente se não tiver solicitacoes na fila")
    void naoDeveEncaminharSolicitacaoParaAtendenteSeNaoTiverSolicitacoes(){

        Equipe equipe = equipe(CARTAO);

        Fila fila = fila();

        Solicitacao solicitacao = SolicitacaoFactory.solicitacao(FINALIZADO);
        solicitacao.setFila(fila);
